...
# --- 3. 模型定义 (保持您提供的 ComplexNet 和 DeepLabV3WithComplexNet 结构) ---
...
# ... (脚本顶部的 import 和文件路径设置、MatrixDataset、ComplexConv2d、ComplexReLU、ComplexNet 定义都保持不变) ...
# ... (假设 ComplexNet 的定义使其输出为 (B, 1024复数, 64, 64)) ...

# --- 3. 模型定义 (修改 DeepLabV3WithComplexNet 以包含ASPP和新的融合策略) ---

class ConvBNReLU(nn.Sequential): # 定义一个辅助模块，方便创建 卷积+BN+ReLU 块
...
class DeepLabV3WithComplexNet(nn.Module):
    def __init__(self, output_channels=2):
        ...
        # 加载完整的预训练DeepLabV3模型，以便分别获取骨干网络和ASPP模块
        ...
        self.encoder_backbone = full_deeplab_model.backbone # ResNet50骨干网络
        self.aspp_module = full_deeplab_model.classifier[0] # ASPP模块 (DeepLabHead的第一个组件)
                                                            # ASPP模块的标准输出是256个通道

        # 路径2：您的ComplexNet
        self.complex_net = ComplexNet() # ComplexNet输出1024个复数通道 -> 2048个实数通道 (拼接后)

        # 特征融合模块定义
        aspp_output_channels = 256 # ASPP模块输出的通道数
        complex_net_real_output_channels = 1024 * 2 # ComplexNet复数输出转为实数后的通道数 (1024实+1024虚)
        
        # 拼接后的总通道数
        concatenated_channels_for_fusion = aspp_output_channels + complex_net_real_output_channels # 256 + 2048 = 2304
        
        # 用于融合和调整通道的1x1卷积层，目标输出512通道给解码器
        self.fusion_conv = ConvBNReLU(concatenated_channels_for_fusion, 512, kernel_size=1, padding=0) 

        # 解码器部分，其输入通道数现在是融合层输出的512
        self.decoder_in_channels = 512 
        self.decoder = nn.Sequential(
            # 输入特征图形状: (B, 512, 64, 64) (假设融合后的特征空间维度是64x64)
            ConvBNReLU(self.decoder_in_channels, 512, kernel_size=3, padding=1), # 可以保持512通道或进一步调整
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, output_channels, kernel_size=1) # 最终输出指定的通道数
        )

    def forward(self, x_input_real_3chan: torch.Tensor) -> torch.Tensor: # 输入是 (B, 3, H_in, W_in) 的实数张量
        # 假设期望的特征融合前的空间尺寸为 (64, 64)
        target_spatial_size_for_fusion = (64, 64) 

        # --- 路径1: DeepLabV3 Backbone + ASPP ---
        # 骨干网络输出，对于512x512输入，标准ResNet50 (output_stride=16) 输出 (B, 2048, 32, 32)
        features_from_backbone = self.encoder_backbone(x_input_real_3chan)['out'] 
        
        # ASPP模块处理骨干网络特征
        x1_from_aspp = self.aspp_module(features_from_backbone) 
        # ASPP输出形状通常是 (B, 256, 32, 32) (空间维度与输入ASPP的特征图一致)
        
        # 将ASPP输出上采样到目标融合空间尺寸
        x1_processed = F.interpolate(x1_from_aspp, size=target_spatial_size_for_fusion, mode='bilinear', align_corners=False)
        # x1_processed 形状: (B, 256, 64, 64)

        # --- 路径2: ComplexNet ---
        x_real_ch = x_input_real_3chan[:, 0, :, :].unsqueeze(1) # (B, 1, 512, 512)
        x_imag_ch = x_input_real_3chan[:, 1, :, :].unsqueeze(1) # (B, 1, 512, 512)
        x_complex_input = CVTensor(x_real_ch, x_imag_ch) # 创建 (B, 1, 512, 512) 复数张量
        
        x2_complex_out = self.complex_net(x_complex_input) # ComplexNet输出: (B, 1024复数通道, 64, 64)
        x2_real_concatenated = torch.cat((x2_complex_out.real, x2_complex_out.imag), dim=1) 
        # x2_real_concatenated 形状: (B, 2048实数通道, 64, 64)

        # --- 特征融合 ---
        # 拼接来自两条路径的特征
        concatenated_features = torch.cat((x1_processed, x2_real_concatenated), dim=1) 
        # concatenated_features 形状: (B, 256 + 2048, 64, 64) = (B, 2304, 64, 64)
        
        # 通过1x1卷积学习融合特征并调整通道数
        fused_features = self.fusion_conv(concatenated_features) 
        # fused_features 形状: (B, 512, 64, 64)
        
        # --- 解码器 ---
        decoded_output = self.decoder(fused_features) # 输出 (B, output_channels, 512, 512)
        
        # 确保最终输出尺寸与标签一致 (如果解码器输出尺寸与输入有微小差异)
        if decoded_output.shape[-2:] != x_input_real_3chan.shape[-2:]: # 比较最终输出和原始输入图像尺寸
             decoded_output = F.interpolate(decoded_output, size=x_input_real_3chan.shape[-2:], mode='bilinear', align_corners=False)
             
        return decoded_output

# --- 后续代码 (calculate_regression_metrics, if __name__ == '__main__': 等) ---
# ... (这部分与我们上一版针对DeepLabV3+ComplexNet修改后的标准化训练脚本一致) ...
# ... (包括 MatrixDataset 的使用，统计量计算，DataLoaders，优化器，训练循环，早停，日志，绘图等) ...
# 只需要将脚本中旧的 DeepLabV3WithComplexNet 类定义替换为上面的新版本即可。
# 确保在 __main__ 中模型实例化时 `model = DeepLabV3WithComplexNet(output_channels=2).to(device)`
...
if __name__ == '__main__':
    model_name = "ComplexNetDeepLab+aspp_Reg" # 模型名称
